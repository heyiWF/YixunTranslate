...
def fix(txt,isname):
    ...
    txt = txt.replace("&It;"," < ")                             # 临时解决小于号输出异常，原因未知
...
def DoTranslate(cookie):
    ...
    data = data['data']
    #print(data['vul_name'])
    #print(data['desc_affected'])
    #print(data['desc_impact'])
    #print(data['desc_summary'])
    #print(data['solu'])
    #pprint(data)
    
    
    #1 Fedora
    if "Fedora Update for" in data['vul_name']:
        # 找到漏洞ID、影响的包、影响的系统
        ref = "FEDORA-"+re.findall(r"FEDORA-(.*)",data['vul_name'])[0]
        affected_app = re.findall(r"Fedora Update for (.*?) FEDORA",data['vul_name'])[0]
        system = re.findall(r".* on (.*)",data['desc_affected'])[0]
        # 构造提交的信息 
...
#        print("成功提交一条！已经成功提交 "+str(SubmitCounter)+" 条已适配格式。")
    ...
    #2 CentOS
    elif "CentOS Update for" in data['vul_name']:

        APPandID = re.findall(r"CentOS Update for (.*)",data['vul_name'])[0]
    ...
    elif "IT-Grundschutz" in data['vul_name']:
        name = driver.find_element_by_name("vul_name_cn")
    ...
    #4 SuSE
    elif "SuSE Update for" in data['vul_name']:
        APPandID = re.findall(r"SuSE Update for (.*)",data['vul_name'])[0]

        affected_app = APPandID.split(" ",1)[0]
        ref = APPandID.split(" ",1)[1]
        cn_vul_name = "SuSE 安全更新 "+ref

        en_summary = data['desc_summary']
        en_affected = data['desc_affected']
        system = re.findall(r".* on (.*)",data['desc_affected'])[0]
        en_solu = data['solu']
    ...
    #5 Ubuntu
    elif "Ubuntu Update for" in data['vul_name']:
        APPandID = re.findall(r" Update for (.*)",data['vul_name'])[0]
        system = re.findall(r".* on (.*)",data['desc_affected'])[0]
    ...
    #5+ RedHat
    elif "RedHat Update for" in data['vul_name']:
        APPandID = re.findall(r" Update for (.*)",data['vul_name'])[0]
        system = re.findall(r".* on (.*)",data['desc_affected'])[0]
    ...
    #6 Local Check
    elif "Local Check" in data['vul_name']:
        en_name = data['vul_name']
    ...
    #7 Debian LTS
    elif "Debian LTS Advisory" in data['vul_name']:
        en_name = data['vul_name']
        en_summary = data['desc_summary']
        en_affected = data['desc_affected']
        en_solu = data['solu']
    ...
    #8 Debian
    elif "Debian Security Advisory" in data['vul_name']:
        en_name = data['vul_name']
        en_summary = data['desc_summary']
        en_affected = data['desc_affected']
        en_solu = data['solu']
    ...
    #9 FreeBSD Ports
    elif "FreeBSD Ports" in data['vul_name']:
        en_solu = data['solu']
    ...
    #10 Gentoo
    elif "Gentoo Security Advisory" in data['vul_name']:
        en_name = data['vul_name']
        en_summary = data['desc_summary']
        en_solu = data['solu']
    ...
    #11 Mandriva
    elif "Mandriva Update for" in data['vul_name']:
        APPandID = re.findall(r"Mandriva Update for (.*)",data['vul_name'])[0]
