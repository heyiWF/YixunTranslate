# -*- coding: utf-8 -*-
# 此脚本依赖Chrome driver，selenium、requests、pymysql和googletrans库
...
# 改成自己的账号密码
username = input("在这里输入你的账号：")
password = input("在这里输入你的密码：")

# 本次执行的条目数
iteration = input("在这里输入你本次要执行自动翻译的条目数(1~100)：")
iteration = int(iteration)
while iteration < 1 or iteration > 100:
    print("请输入1到100之间的数值！")
    iteration = input("在这里输入你本次要执行自动翻译的条目数(1~100)：")
    iteration = int(iteration)
...
def Translate(driver,cookie):
    ...
    print("\n执行完毕。本次共翻译{0}条，其中{1}条已提交，还有{2}条未提交内容在“翻译列表”栏内。".format
...
def TransSolu(translator,solu):
    ...
    if solu == "Mitigation" or solu == "Null":
        solu = "空。"
        ...
        solu = rep4
    elif "Update the affected" in solu:
        solu = rep5
        ...
        solu = rep11
    elif solu == "Ask the Vendor for an update.":
        solu = rep12
    elif "http" in solu:
        solu = solu.replace("oldstable","old stable")
        sitestart = solu.find("http")                #找到http开头的位置
        siteend = solu.find(" ",sitestart+1)         #找到网址结尾的位置，如果返回值为-1则说明网址后面没有内容了
        if sitestart == 0 :
            solu = fix(solu,1)                       #纯网址不加句号
            pass                                     #开头就是网址，solu不做修改直接复制
        elif siteend == -1 :                         #网址在末尾
            solu = translator.translate(solu[0:sitestart],dest='zh-cn').text + solu[sitestart:]
            solu = fix(solu,0)
        else :                                       #网址在中间
            solu_1 = solu[0:sitestart]
            solu_2 = solu[siteend+1:]
            webline = solu[sitestart:siteend+1]      #把网址切出来
    ...
    solu = solu.replace("对于Debian 7\'","对于Debian 7 \'")
    solu = solu.replace("对于Debian 8\'","对于Debian 8 \'")
    solu = solu.replace(" + ","+")
    solu = solu.replace("：",":")
    solu = solu.replace("its安全跟踪器页面:","安全跟踪页面：")
    solu = solu.replace("的分布","发行版")
    solu = solu.replace("分布","发行版")
    solu = solu.replace("的分发","发行版")
    solu = solu.replace("分发","发行版")
    solu = solu.replace("version","版本")
    solu = solu.replace("中得到修复","中修复")
    if "这个问题" in solu and "此问题" in solu:
        solu = solu.replace("这个问题","此问题")
...
def fix(txt,isname):
    ...
    txt = txt.replace("XXE","XML外部实体注入")
    txt = txt.replace("BOF","缓冲区溢出")
    txt = txt.replace("跨站点","跨站")
    txt = txt.replace("并且容易出现","容易出现")
    txt = txt.replace("NVT","漏洞规则")
    txt = txt.replace("影响程度：申请","影响程度：应用程序")
    txt = txt.replace("多个漏洞","多个安全漏洞")
    txt = txt.replace("宣布","发布")
    txt = txt.replace("未指定","未明")
    txt = txt.replace("Common Vulnerabilities and Exposures","CVE")
    txt = txt.replace("（）函数","()函数")
    txt = txt.replace("使用后免费","释放后重用")
    txt = txt.replace("免费后使用","释放后重用")
    txt = txt.replace("免费使用后","释放后重用")
    txt = txt.replace("远程执行代码漏洞","远程代码执行漏洞")
    txt = txt.replace("该主机运行的是","该主机正在运行")
    txt = txt.replace("输入清理","输入验证")
    txt = txt.replace("谷歌Chrome","Google Chrome")
    txt = txt.replace("开放票务请求","开源工单请求")
    txt = txt.replace("铬","chromium")
    txt = txt.replace("几个漏洞","多个漏洞")
    txt = txt.replace("星号","Asterisk")
    txt = txt.replace("思科","Cisco")
    txt = re.sub(r"(?i)MitM攻击","中间人攻击",txt)
    # 中文标点改为英文
    txt = txt.replace("#","#")
    txt = txt.replace("！","!")
    if isname:
...
def DoTranslate(cookie):
    ...
    #1 Fedora
    if "Fedora Update for" in en_name:
        # 找到漏洞ID、影响的包、影响的系统
        ref = "FEDORA-"+re.findall(r"FEDORA-(.*)",en_name)[0]
        affected_app = re.findall(r"Fedora Update for (.*?) FEDORA",en_name)[0]
        en_affected = en_affected.strip(".")
    ...
    #2 CentOS
    elif "CentOS Update for" in en_name:
        APPandID = re.findall(r"CentOS Update for (.*)",en_name)[0]
    ...
    elif "IT-Grundschutz" in en_name:
        name = driver.find_element_by_name("vul_name_cn")
    ...
    #4 SuSE/Mandriva
    elif "SuSE Update for" in en_name or "Mandriva Update for" in en_name:
        APPandID = re.findall(r" Update for (.*)",en_name)[0]
        sys = re.findall(r"(.*) Update for ",en_name)[0]
        en_affected = en_affected.strip(".")
        system = re.findall(r".* on (.*)",en_affected)[0]

        affected_app = re.findall(r"\((.*?)\)",APPandID)[0]
        ref = re.findall(r" (.*?) \(",APPandID)[0]
        cn_vul_name = sys+" 安全更新 "+ref+"（"+affected_app+"）"
    ...
    #5 Ubuntu/RedHat
    elif "Ubuntu Update for " in en_name or "RedHat Update for" in en_name:
        APPandID = re.findall(r" Update for (.*)",en_name)[0]
        sys = re.findall(r"(.*) Update for ",en_name)[0]
        en_affected = en_affected.strip(".")
        system = re.findall(r".* on (.*)",en_affected)[0]
    ...
    #7 Debian LTS
    elif "Debian LTS Advisory" in en_name:
        en_affected = en_affected.strip(".")
    ...
    #8 Debian
    elif "Debian Security Advisory" in en_name:
        if " on " in en_affected:
            en_affected = en_affected.strip(".")
    ...
    #10 Gentoo
    elif "Gentoo Security Advisory" in en_name:
        ...
        if "users should upgrade" in en_solu:
            affected_app = re.findall(r"All (.*?) users",en_solu)[0]
            line = "所有"+affected_app+"用户须升级至最新版本："
            cn_solu = re.sub(r"All .*?:",line,en_solu)
            cn_solu = fix(cn_solu,0)
        else:
            cn_solu = TransSolu(translator,en_solu)
    ...
    #11 Detection
    elif "Detection" in en_name and not "Version Detection" in en_name:
        app = re.findall(r"(.*) Detection",en_name)[0]
        cn_name = "检测到"+app
        cn_summary = translator.translate(en_summary,dest='zh-CN').text
        cn_summary = fix(cn_summary,0)
        cn_solu = TransSolu(translator,en_solu)
    ...
    #12 Slackware Advisory
    elif "Slackware Advisory" in en_name:
    ...
    #13 openSUSE
    elif "openSUSE Update for" in en_name:
        APPandID = re.findall(r" Update for (.*)",en_name)[0]
        en_affected = en_affected.strip(".")
        system = re.findall(r".* on (.*)",en_affected)[0]
        ...
        affected_app = re.findall(r"\((.*?)\)",APPandID)[0]
        ref = re.findall(r" (.*?) \(",APPandID)[0]
        cn_vul_name = "openSUSE 安全更新 "+ref+"（"+affected_app+"）"
...
if __name__ == "__main__":
    ...
    print("登录成功，开始翻译。如果中途程序报错，请在群里告诉我们以便修复，谢谢！")
    # 第三个参数是循环次数，可手动修改
    Translate(driver,cookie)
    driver.quit()
